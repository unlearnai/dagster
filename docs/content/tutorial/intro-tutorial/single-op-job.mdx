...
object="OpExecutionContext" displayText="context" /> in which it executes (provided by the Dagster framework as the first argument to ops who specify it), and also returns no outputs. Don't worry, we'll soon encounter ops that are much more dynamic.
